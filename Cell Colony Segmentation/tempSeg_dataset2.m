...
%%
% sourcepath  = {'C:\Users\Delmon Arous\Desktop\Images\Dataset1'};
...
destpath    = 'C:\Users\Delmon Arous\Desktop\Results\Dataset2';
...
dpi = 314; % 1200 for celler og 314 for bakterier
param.px_size = 25.4/dpi;

% Define a search interval for colony area
area = [700 20000]; % 75 for T47D cancer cell mean diameter 20-30 um
...
filelist        = getAllFiles(sourcepath{2});
...
for i = 1:length(filelist)
    ...
    param.gaussfilt_size_pca    = gaussfilt_size_kmeans{i};     % [2 2]; %
    param.obrcbr_size_pca       = obrcbr_size_kmeans{i};        % 25;    %
    param.gaussfilt_size        = gaussfilt_size_watershed{i};  % [3 3]; %
    param.obrcbr_size           = obrcbr_size_watershed{i};     % 6;     %
    
    if usejava('jvm') && feature('ShowFigureWindows')
        
        % Start progress bar
        progress = uiprogressdlg(f1, 'Title', ...
    ...
    if param.extractcontainer == 1
        bw_border = getCellContainerBorder(filelist_dish{i} , ... % sourcepath_img_dish{1}
            img_norm, f1);
    else
    ...
    visboundaries(bw_seg_PCA, 'Color', 'magenta', 'LineWidth', 2)
