...
sourcepath = 'C:\Users\delmo\Desktop\Count\A549';
...
%     figure();
%     hold on
%     scatter(manual, auto, [], dose, 'filled', 'MarkerEdgeColor', 'black');
...
%     lgd = legend(['T-47D data, Exp ' num2str(i)], 'Location', 'NorthWest');
...
%     set(gca, 'FontSize', 14)    
    ...
    %% Regression
    tbl_auto = table(dose, log(SF_auto), 'VariableNames', {'D', 'SF'});
    tbl_manual = table(dose, log(SF_manual), 'VariableNames', {'D', 'SF'});
    formula = 'SF ~ -1 + D + D^2';
    lm_auto = fitlm(tbl_auto, formula)
    lm_manual = fitlm(tbl_manual, formula)
    
    figure();
    hold on
    plot(dose, exp(lm_auto.Fitted), '-r', dose, exp(lm_manual.Fitted), '-b', ...
        dose, SF_auto, 'or', dose, SF_manual, 'xb')
%     plot(dose, exp(lm_auto.Fitted), '-', dose, SF_auto, 'o')
    hold off
    set(gca,'YScale','log')
    lgd = legend('Automatic, LQ fit', 'Manual, LQ fit', ...
        'Automatic, SF data', 'Manual, SF data', 'Location', 'SouthWest');
%     lgd = legend('Automatic, LQ fit', 'Automatic, SF data', ...
%         'Location', 'NorthEast');    
    title(lgd, ['A549, Exp ' num2str(i)])
    xlim([-0.1 max(doses)+0.1])
    xlabel('Dose (Gy)')
