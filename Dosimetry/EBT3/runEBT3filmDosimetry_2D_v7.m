...
for n = 1:N
    ...
    for k = 1:K
        ...
        % Clean Gradient vectors from Inf and NaN values
        % Problem for Open field: peak-to-valley and valley-to-peak
        % distance quadrats are all zero --> making the gradient quadrat
        % values Inf!!!!
        any(isnan(X_G_train.peak))
        any(isinf(X_G_train.peak))  % Check which values for X_PV_R_train and X_VP_R_train are included in the analysis!!!!!
        any(isnan(X_G_train.valley))
        any(isinf(X_G_train.valley))
        any(isnan(X_G_test.peak))
        any(isinf(X_G_test.peak))
        any(isnan(X_G_test.valley))
        any(isinf(X_G_test.valley))

        x =lsqnonlin(@(params) MLQ_twocomp(params, ...
            X_D_train.peak, X_D_train.valley, ...
            X_G_train.peak, X_G_train.valley, ...
            Y_SF_train.peak, Y_SF_train.valley), ...
            [0.24, 0.019, 0.001, 0.001], [0, 0, -1, -1], [1, 1, 1, 1]);

        
        %         res = MLQ_twocomp([0, 0, 0, 0], ...
        %             X_D_train.peak, X_D_train.valley, ...
        %             X_G_train.peak, X_G_train.valley, ...
        %             Y_SF_train.peak, Y_SF_train.valley);
        %
        ...
        %         res = MLQ_twocomp([0.1, 0.01, 0.00001, 0.00001], ...
        %             X_D_train.peak, X_D_train.valley, ...
        %             X_G_train.peak, X_G_train.valley, ...
        %             Y_SF_train.peak, Y_SF_train.valley);

        %         fun = @(params) [ ...
        %             Y_SF_train.peak - exp(params(1).*X_D_train.peak + params(2).*((X_D_train.peak).^2) + params(3).*X_G_train.peak), ...
        %             Y_SF_train.valley - exp(params(1).*X_D_train.valley + params(2).*((X_D_train.valley).^2) + params(4).*X_G_train.valley)];

        %         x = lsqnonlin(fun, [0.1, 0.01, 0.00001, 0.00001]')
