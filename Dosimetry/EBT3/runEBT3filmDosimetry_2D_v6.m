...
peakdose.Control      = 0.0;
peakdose.GRID_Dots    = 0.70;
peakdose.GRID_Stripes = 0.82;
peakdose.Open         = 1.0;
...
for i = 1:length(folderlist_assay)
    ...
    if i > 1
        ...
        temp_img_peakarea = struct.(assay_pattern{i}).img_peakarea;
...
for i = 1:length(assay.Control.img)
    ...
    N_colonies_ctrl = [N_colonies_ctrl assay.Control.img{i}.N_colonies_uncalib];
...
for n = 1:N
    ...
    for k = 1:K
        ...
        for j = 1:length(assay_pattern)
            ...
            peakdist_vec_train.(assay_pattern{j}) = [];
            count_vec_train.(assay_pattern{j})    = [];
            dose_vec_test.(assay_pattern{j})      = [];
            peakdist_vec_test.(assay_pattern{j})  = [];
            ...
            for i = 1:length(assay_train.(assay_pattern{j}).img)
                ...
                peakdist_vec_train.(assay_pattern{j}) = [peakdist_vec_train.(assay_pattern{j}) peakdist_quadrat_PV'];
            ...
            for i = 1:length(assay_test.(assay_pattern{j}).img)
                ...
                peakdist_quadrat_PV = assay_test.(assay_pattern{j}).img{i}.PV.peakdist_quadrat;
                ...
                peakdist_vec_test.(assay_pattern{j}) = [peakdist_vec_test.(assay_pattern{j}) peakdist_quadrat_PV'];
            ...
            X_R_train  = [X_R_train  peakdist_vec_train.(assay_pattern{j})];
            Y_CC_train = [Y_CC_train count_vec_train.(assay_pattern{j})];
            X_D_test   = [X_D_test   dose_vec_test.(assay_pattern{j})];
            X_R_test   = [X_R_test   peakdist_vec_test.(assay_pattern{j})];
        ...
        tbl_LQ    = table(X_D_train(:), Y_CC_train(:), 'VariableNames', {'D', 'Y_CC'});
        tbl_MLQ = table(X_D_train(:), X_R_train(:), Y_CC_train(:), 'VariableNames', {'D', 'R', 'Y_CC'});
        ...
        R_train          = linspace(min(X_R_train(:)), max(X_R_train(:)), 101);
...
%         plot3Dsurf(X_D_train, X_R_train, Y_CC_train, D_mesh, R_mesh, ...
%             reshape((yhat_train_LQ),101,101), ...
%             reshape((yhat_train_MLQ),101,101), ...
%             'Distance from peak (cm)', ...
%             'MLQ fit $(\exp(\lambda_0 -\alpha D - \beta D^2 - \delta DR))$', ...
%             fullfile(path, sprintf('SurfPlot_LQvsMLQ_%s.png', num2str(k))))
        ...
        [yhat_test_MLQ, ci_test_MLQ] = predict(mdl_MLQ, [X_D_test(:), X_R_test(:)]);
...
for i = 1:length(assay_dose)
    for j = 2:3 % 1:length(assay_pattern)
        for k = 1:length(GRID_region)
            ...
            R_temp  = irrad_dose.(assay_dose{i}).(assay_pattern{j}).(GRID_region{k}).dist;
